...
const authController = {
 ...
 loginUser: async (req, res) => {
  ...
  const { user_ln, user_password } = req.body;

  // ✅ Validate input
  if (!user_ln || !user_password) {
  ...
  db.query(sql, [user_ln], async (err, result) => {
    ...
    if (result.length === 0) {
      console.log("User not found:", user_ln);
    ...
    try {
      ...
      console.log(`User ${user_ln} logged in successfully as ${user.user_role}`);
